...
val_set = bladder.Bladder('./traindata', 'val',
...
if loss_name == 'dice_':
    criterion = SoftDiceLoss(num_classes=4,activation='sigmoid').cuda()
#elif loss_name == 'bce_':
...
def val(model):
    imname = 'Image_NOR05No.5'
    # imname = '2-IM107'
    img = Image.open('D:\heartseg\seg2/traindata\image/{}.png'.format(imname))
    mask = Image.open('D:\heartseg\seg2/traindata\label/{}.png'.format(imname))
    ...
    pred[pred < 0.5] = 0
    pred[pred > 0.5] = 1
    bladder_dice = diceCoeffv2(pred[:, 0:1, :], gt.cuda()[:, 0:1, :], )
    tumor_dice = diceCoeffv2(pred[:, 1:2, :], gt.cuda()[:, 1:2, :], )
...
def auto_val(model):
    ...
    for i, (img, mask) in enumerate(val_loader):
        ...
        iters += batch_size
        #dice 指标
        bladder_dice = diceCoeff(pred[:, 0:1, :], mask[:, 0:1, :], )
        tumor_dice = diceCoeff(pred[:, 1:2, :], mask[:, 1:2, :],)
        heart_dice = diceCoeff(pred[:, 2:3, :], mask[:, 2:3, :],)
        mean_dice = (bladder_dice + tumor_dice+heart_dice) / 3

        dices += mean_dice
        tumor_dices += tumor_dice
        bladder_dices += bladder_dice
        heart_dices+= heart_dice

        acc = accuracy(pred, mask)
        p = precision(pred, mask)
        r = recall(pred, mask)
        #f1score= 2*
        print('mean_dice={:.4}, bladder_dice={:.4}, tumor_dice={:.4}, heart_dice={:.4}, acc={:.4}, p={:.4}, r={:.4}'
              .format(mean_dice.item(), bladder_dice.item(), tumor_dice.item(), heart_dice.item(),
                      acc, p, r))
    ...
    val_heart_dice = heart_dices / (len(val_loader) / batch_size)
    print('Val Mean Dice = {:.4}, Val Bladder Dice = {:.4}, Val Tumor Dice = {:.4}, Val heart Dice={:.4}'
          .format(val_mean_dice, val_bladder_dice, val_tumor_dice, val_heart_dice))
